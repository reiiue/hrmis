    ...
    <tr>
        ...
        <td colspan="14">
            <input type="text" 
                name="position_title[]" 
                class="form-control pds-input-borderless"
                value="{{ old('position_title.' . $work->id, $work->position_title) }}">
        </td>
        ...
        <td colspan="14">
            <input type="text" 
                ...
                class="form-control pds-input-borderless"
                value="{{ old('department_agency.' . $work->id, $work->department_agency) }}">
        </td>
        ...
        <td colspan="7">
            <input type="number" 
                name="monthly_salary[]" 
                class="form-control pds-input-borderless"
                value="{{ old('monthly_salary.' . $work->id, $work->monthly_salary) }}">
        </td>
        ...
        <td colspan="5">
            <input type="text" 
                ...
                class="form-control pds-input-borderless"
                value="{{ old('salary_grade_step.' . $work->id, $work->salary_grade_step) }}">
        </td>
        ...
        <td colspan="6">
            <input type="text" 
                ...
                class="form-control pds-input-borderless"
                value="{{ old('status_appointment.' . $work->id, $work->status_appointment) }}">
        </td>

        {{-- Gov’t Service --}}
        <td colspan="4">
            <select name="gov_service[]" class="form-control pds-input-borderless">
                <option value="Y" {{ old('gov_service.' . $work->id, $work->gov_service) == 'Y' ? 'selected' : '' }}>Y</option>
                <option value="N" {{ old('gov_service.' . $work->id, $work->gov_service) == 'N' ? 'selected' : '' }}>N</option>
            </select>
        </td>
    </tr>
    @endforeach
